    ...
    def walk(self):
        ...
        for name, path in self._walk_folder():
            if os.path.isdir(path) and name.lower().startswith('bias'):
                ...
                self.biases_folders.append(FilesWalker(self.context, self.astronomer, path, self.prefix))
            elif os.path.isdir(path) and name.lower().startswith('dark'):
                if self.context.debug: print(f' >  > Found a [{self.prefix}] {name} folder.')
                self.darks_folders.append(FilesWalker(self.context, self.astronomer, path, self.prefix))
